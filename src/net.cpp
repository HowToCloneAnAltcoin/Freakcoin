...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("WINTERCOIN-ext-ip");
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "WINTERCOIN " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
    {"104.131.64.78", "104.131.64.78"},
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("WINTERCOIN-dnsseed");
...
void ThreadDumpAddress(void* parg)
{
    ...
    RenameThread("WINTERCOIN-adrdump");
...
void ThreadOpenConnections(void* parg)
{
    ...
    RenameThread("WINTERCOIN-opencon");
...
void ThreadMessageHandler(void* parg)
{
    ...
    RenameThread("WINTERCOIN-msghand");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. WINTERCOIN is probably already running."), addrBind.ToString().c_str());
        else
